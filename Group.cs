   public class Group
    {
        ...
        public List<string> GetChoresDueThisWeek()
        {
            List<string> tasks = new List<string>();
            TaskList choreList = GetTaskList();
            Timestamp time = new Timestamp();
            foreach (Chore chore in choreList.GetChoreList())
            {
                // Check if within 7 days of current day
                if (chore.GetDeadline().GetDay() >= time.CurrentTimestamp().GetDay() && chore.GetDeadline().GetDay() <= time.CurrentTimestamp().GetDay() + 7)
                {
                    tasks.Add(chore.GetName());
                }
            }
